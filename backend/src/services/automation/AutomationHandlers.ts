...
export class AutomationHandlers {
	...
	async handleLightAutomation(lightLevel: number, now: number): Promise<void> {
		...
		try {
			if (lightLevel <= configData.lightThresholds.turnOnWhenDark) {
				await this.deviceController.controlDevice('light', 'on', `Light level too low: ${lightLevel} <= ${configData.lightThresholds.turnOnWhenDark}`);
				this.lastLightAutomation = now;
			} else if (lightLevel >= configData.lightThresholds.turnOffWhenBright) {
				await this.deviceController.controlDevice('light', 'off', `Light level too high: ${lightLevel} >= ${configData.lightThresholds.turnOffWhenBright}`);
				this.lastLightAutomation = now;
			}
		} catch (error) {
	...
	async handlePumpAutomation(soilMoisture: number, now: number): Promise<void> {
		...
		try {
			if (soilMoisture <= configData.pumpThresholds.turnOnWhenDry) {
				await this.deviceController.controlDevice('pump', 'on', `Soil moisture too low: ${soilMoisture} <= ${configData.pumpThresholds.turnOnWhenDry}`);
				this.lastPumpAutomation = now;
			} else if (soilMoisture >= configData.pumpThresholds.turnOffWhenWet) {
				await this.deviceController.controlDevice('pump', 'off', `Soil moisture too high: ${soilMoisture} >= ${configData.pumpThresholds.turnOffWhenWet}`);
				this.lastPumpAutomation = now;
			}
		} catch (error) {
		...
		try {
			if (temperature >= configData.temperatureThresholds.windowOpenTemp) {
				if (configData.doorControlEnabled && temperature >= configData.temperatureThresholds.doorOpenTemp && (now - this.lastDoorAutomation >= this.AUTOMATION_COOLDOWN)) {
					await this.deviceController.controlDevice('door', 'open', `Temperature too high: ${temperature} >= ${configData.temperatureThresholds.doorOpenTemp}`);
					this.lastDoorAutomation = now;
				}

				if (configData.windowControlEnabled && (now - this.lastWindowAutomation >= this.AUTOMATION_COOLDOWN)) {
					await this.deviceController.controlDevice('window', 'open', `Temperature too high: ${temperature} >= ${configData.temperatureThresholds.windowOpenTemp}`);
				...
				if (configData.doorControlEnabled && temperature <= configData.temperatureThresholds.doorCloseTemp && (now - this.lastDoorAutomation >= this.AUTOMATION_COOLDOWN)) {
					await this.deviceController.controlDevice('door', 'close', `Temperature too low: ${temperature} <= ${configData.temperatureThresholds.doorCloseTemp}`);
					this.lastDoorAutomation = now;
				}

				if (configData.windowControlEnabled && (now - this.lastWindowAutomation >= this.AUTOMATION_COOLDOWN)) {
					await this.deviceController.controlDevice('window', 'close', `Temperature too low: ${temperature} <= ${configData.temperatureThresholds.windowCloseTemp}`);
					this.lastWindowAutomation = now;
				}
			}
		} catch (error) {
	...
	async handleRainAutomation(rainStatus: number, now: number): Promise<void> {
		...
		try {
			if (rainStatus === 1) {
				await this.deviceController.controlDevice('window', 'close', 'Rain detected - closing window for protection');
				this.lastWindowAutomation = now;
			}
		} catch (error) {
	...
	async handleWaterLevelAutomation(waterLevel: number, now: number): Promise<void> {
		...
		try {
			if (waterLevel === 1) { // 1 = flooded
				await this.deviceController.controlDevice('pump', 'off', `Water level flooded: ${waterLevel} - emergency pump shutdown`);
				this.lastPumpAutomation = now;
			}
		} catch (error) {
